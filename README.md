...
- actions: defines the change that needs to be implemented
  reducers: carry out the actions, such as "add_todo or remove_todo". it contains the formula (...state or usage of .map so that dispatch can carry out the function)
  store: holds the reducer, making it more accessible for other components to use.

3. What is the difference between Application state and Component state? When would be a good time to use one over the other?
   -Application state is the 'global' state that all components can access. it is connected with index.js which renders onto the page. components are used for organizing functions into separate files; it isn't good practice to place all code in App.js
